...
(function(global, undefined) {
	...
	viewlet.apply = function(vl) {
		if (vl.output && vl.template && vl.template.html) {

			if (vl.datalet && vl.datalet.name) {
				viewlet.datalet(vl, function(err, data) {
					if (err) {
						return;
					}
					console.log(data);
					...
					valueui.template.render({
						...
						dstid: "valueui-" + vl.output,
						data: data,
						callback: function() {
							viewlet.next(vl.next_viewlets);
						},
					});
				});
			} else {
	...
	viewlet.run = function(vl) {
		if (!vl || !vl.name) {
			return;
		}
		var url = valueui.basepath + "/api/v2/viewlet/fetch?name=viewlet/" + vl.name + ".toml";
	...
	viewlet._datasetConvert = function(data) {
		if (!data.rows || !data.spec || !data.spec.fields) {
			return;
		}
		var sets = {
			name: data.name,
		...
		for (var i in data.rows) {
			var row = data.rows[i];
			if (data.spec.fields.length != row.values.length) {
				continue;
			}
			var row2 = {}
			for (var j in data.spec.fields) {
				row2[data.spec.fields[j].name] = row.values[j];
			}
			sets.rows.push(row2)
		}
		return sets;
	...
	viewlet.datalet = function(vl, cb) {
		if (!vl || !vl.template || !vl.template.html || !vl.output || !vl.datalet || !vl.datalet.name) {
			return;
		}
		...
		var url = valueui.basepath + "/api/v2/datalet/run?name=datalet/" + vl.datalet.name + ".toml";
		...
		valueui.utilx.ajax(url, {
			callback: function(err, data) {
				...
				for (var i in data.datasets) {
					if (data.datasets[i].name == vl.datalet.name) {
						return cb(null, viewlet._datasetConvert(data.datasets[i]));
					}
				}
			},
		});
		...
		emit(
			...
			(emitData = {
				...
				crossorigin: isFunction(data.crossorigin) ?
					data.crossorigin(requestUri) : data.crossorigin,
			})
		);
